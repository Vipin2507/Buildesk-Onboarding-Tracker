...
const COPY = {
  crm: {
    ...
    intro:
      "Connect your Google account so approved meetings create a Calendar event with a Meet link, and your Google busy times block open slots.",
    connectBody:
      "When you approve a meeting, Buildesk creates a Google Calendar event with Meet and invites the guest. While connected, Google busy times are removed from bookable slots.",
    footer:
      "Approving a meeting creates a Meet link and emails the guest. Cancel / decline removes the calendar event.",
    howItWorks: [
      "Guest books a pending slot in the portal.",
      "You approve in CRM Meetings → Google creates the event + Meet.",
      "Guest receives the Meet link in the status email (and on their portal).",
    ],
  },
  erp: {
    title: "Google Calendar & Meet",
    intro:
      "Connect your Google account so scheduled online ERP meetings create Calendar events with Meet links for your team and clients.",
    connectBody:
      "When you schedule an online meeting, Buildesk creates a Google Calendar event with Meet and invites the attendee when an email is provided.",
    footer:
      "Online meetings sync to Google Calendar. Cancelling or marking no-show removes the calendar event.",
    howItWorks: [
      "Schedule an online meeting and assign a host.",
      "Buildesk creates the Google event + Meet link on save.",
      "Share the Meet link with the client or open it from the meeting row.",
    ],
  },
} as const;

export function GoogleCalendarConnectPanel({ variant, flash, flashError }: Props) {
  ...
  async function connect() {
    setBusy(true);
    try {
      const { url } = await getGoogleCalendarAuthUrl({ data: { returnTo: variant } });
  ...
  return (
    <div className="space-y-3">
      <DesignTicketSection compact title={copy.title}>
        <p className="mb-2 text-[10px] text-muted-foreground">{copy.intro}</p>
      </DesignTicketSection>
        ...
        <div className="card-soft space-y-3 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">{copy.connectBody}</p>
            </div>
          </div>
          <Button
            type="button"
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1 text-[10px] text-muted-foreground">
                  ...
                  {status.connectedAt ? ` · since ${status.connectedAt.slice(0, 10)}` : null}
                </div>
              </div>
            </div>
