...
import {
  notifyCrmAutomationResult,
  testCrmAutomationChannel,
  testCrmAutomationRule,
...
const TRIGGER_SHORT: Record<AutomationRule["trigger"], string> = {
  ...
  "portal-ticket-created": "Portal ticket",
};
    ...
    try {
      ...
      const log =
        mode === "rule" && selectedRule
          ? await testCrmAutomationRule(selectedRule.id, overrides)
          : await testCrmAutomationChannel(mode as AutomationChannel, overrides);
      ...
      notifyCrmAutomationResult(log, mode === "rule" ? selectedRule?.name ?? "Test" : `${mode} test`);
    } catch (err) {
            ...
            {endpoints.map((ep) => (
              <Button
                ...
                disabled={
                  sending ||
                  !ep.isEnabled ||
                  (ep.channel === "whatsapp" && !useCrmAutomationStore.getState().waha.isEnabled)
                }
