  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-xs font-semibold text-muted-foreground">Client visits</h3>
        ...
        {canManage ? (
          <Button size="sm" className="h-7 gap-1 text-xs" onClick={openCreate}>
            <Plus className="h-3 w-3" /> Log visit
          </Button>
        ) : null}
      </div>
      ...
      {companyVisits.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {companyVisits.map((visit) => (
            <div key={visit.id} className="card-soft space-y-1.5 px-3 py-2">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <button type="button" className="min-w-0 text-left" onClick={() => canManage && openEdit(visit)}>
                  <div className="text-sm font-medium">{visit.purpose}</div>
              ...
              {visit.outcome ? (
                <p className="text-xs text-muted-foreground">Outcome: {visit.outcome}</p>
              ) : null}
              {visit.nextAction ? (
                <p className="text-xs">Next: {visit.nextAction}
                  {visit.nextFollowUpDate ? ` by ${formatDate(visit.nextFollowUpDate)}` : ""}
                </p>
              ) : null}
              {canManage && visit.status === "completed" ? (
                <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => createFollowUp(visit)}>
