...
const DEFAULT_FORM: RuleFormState = {
  ...
  channel: "email",
  isActive: true,
  templateSubject: "Update for ticket {{ticketNumber}} — {{companyName}}",
  templateBody:
...
function PreviewPanel({ form }: { form: RuleFormState }) {
  const globalCc = useAutomationStore((s) => s.settings.emailCc);
  const previewBody = useMemo(
    () => renderAutomationTemplate(form.templateBody, AUTOMATION_SAMPLE_VARS),
    [form.templateBody],
  );
  const previewSubject = useMemo(
    () => renderAutomationSubject(form.templateSubject, AUTOMATION_SAMPLE_VARS),
          ...
          {form.channel === "email" ? (
            <div className="space-y-2 rounded-lg border border-border/50 bg-background/80 p-3 text-sm">
              <div>
                ...
                <div className="mt-0.5">{AUTOMATION_SAMPLE_VARS.customerName} &lt;customer@example.com&gt;</div>
              </div>
              {mergedCc ? (
                <div>
...
export function AutomationRuleDialog({
  ...
  async function handleTest() {
    ...
    try {
      ...
      const log = await testAutomationRule(ruleId, {
      ...
      notifyAutomationResult(log, form.name);
      onOpenChange(false);
      onSaved?.();
    } finally {
      setTesting(false);
                    ...
                    >
                      <Field label="Email subject" required hint="Supports template variables.">
                        <Input
                          ...
                          placeholder="Ticket {{ticketNumber}} — {{companyName}}"
                        />
                      </Field>
                      <Field
                        label="CC recipients"
