...
function scheduleChecklistSync(projectId: string, markAll?: boolean) {
  queueMicrotask(() => {
    void import("@/lib/progress-onboarding-bridge").then((m) => {
      if (markAll !== undefined) {
        m.syncMarkAllToChecklist(projectId, markAll);
      } else {
        m.syncChecklistFromProgress(projectId);
      }
    });
  });
}
...
export const useProjectProgressStore = createStore<ProjectProgressState>((set, get) => ({
  ...
  toggleCheck: (projectId, key) => {
    ...
    scheduleChecklistSync(projectId);
  },
  ...
  markAll: (projectId, value) => {
    ...
    scheduleChecklistSync(projectId, value);
  },
