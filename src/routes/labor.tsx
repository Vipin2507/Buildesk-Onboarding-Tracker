              ...
              {attendance.map((a) => (
                <div key={a.id} className="flex items-center justify-between gap-2 rounded-lg border border-border p-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{formatRelativeTime(a.uploadedAt)} · {a.recordCount} records</div>
                  </div>
                  <Button size="icon" variant="ghost" onClick={() => deleteAttendance(a.id)}><Trash2 className="h-4 w-4" /></Button>
                </div>
              ))}
            </div>
